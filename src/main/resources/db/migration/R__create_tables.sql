...
ALTER TABLE `patient` MODIFY `dob` varchar(10);
...
ALTER TABLE `pat` ADD `dob` date NOT NULL;
COMMIT;
...
CREATE or replace DEFINER=`root`@`localhost` PROCEDURE `add_user` (IN `email` VARCHAR(100),
        ...
        IF NOT EXISTS (SELECT user.email FROM user WHERE user.email = email) THEN
			IF ( role = 'ADMIN' ) THEN
                START TRANSACTION;
...
CREATE or replace DEFINER=`root`@`localhost` PROCEDURE `add_patient` (IN `nic` varchar(50),
        ...
        IF NOT EXISTS (SELECT * FROM patient WHERE patient.nic = nic AND patient.first_name = first_name AND patient.last_name = last_name) THEN
            START TRANSACTION;
            INSERT INTO `patient` (`nic`, `address`,`gender`,`age`,`contact_no`,`is_user`,`hospital_id`,`first_name`,`last_name`,`is_child`,`dob`)
            VALUES (nic,address,gender,age,contact_no,is_user,hospital_id,first_name,last_name,is_child,dob);
